...
ASN=$1
whois -h whois.registro.br AS$ASN | awk 'match($0,/((25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)/) {print substr($0,RSTART,RLENGTH)}' > /tmp/bgp_$ASN
echo "ASN: $ASN"
...
do
  ...
  -H 'Accept: text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8' \
  -H 'Origin: http://lg.level3.net' --data "sitename=ear1.ams1&address=$PREFIX&length=22" \
